...
// dd($response);
?>
<!--[if BLOCK]><![endif]--><?php if($response != null): ?>
  ...
  <!-- Tombol untuk Membuka Modal -->
    <button style="font-size: 12px; background-color: #4A5568; color: white; font-weight: bold; padding: 4px 8px; border-radius: 4px; border: none; cursor: pointer;" onclick="openModal()" type="button">
        <span style="color: inherit; text-decoration: none;">
            <?php echo TranslationHelper::translateIfNeeded('Add New Project'); ?>
    ...
    <div class="fixed active inset-0 flex justify-center z-50" style="max-height: 80%">
        <div class="relative space-y-6 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-4xl w-full max-h-[80%] overflow-y-auto mx-4 md:mx-auto">
            ...
            <h2 class="text-center text-lg font-semibold text-gray-900 dark:text-white">
                <?php echo TranslationHelper::translateIfNeeded('Create Project'); ?>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Case Input -->
                    <div>
                        <label class="block text-gray-700 dark:text-gray-300"><?php echo TranslationHelper::translateIfNeeded('Case'); ?></label>
                    ...
                    <!-- Revenue Input -->
                    <div>
                        <label class="block text-gray-700 dark:text-gray-300"><?php echo TranslationHelper::translateIfNeeded('Revenue'); ?></label>
                    ...
                    <!-- Currency Select -->
                    <div>
                        <label class="block text-gray-700 dark:text-gray-300"><?php echo TranslationHelper::translateIfNeeded('Currency'); ?></label>
                        <select id="currencies_id" name="currencies_id" class="w-full mt-1 p-2 border dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 rounded-md focus:ring focus:ring-blue-500">
                            ...
                            ?>
                            <option value="" disabled selected><?php echo TranslationHelper::translateIfNeeded('Choose a currency'); ?></option>
                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $currencies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $currency): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($id); ?>" <?php echo e($defaultCurrencyId == $id ? 'selected' : ''); ?>>
                                    <?php echo e($currency); ?>
                    ...
                    <!-- Customer Select -->
                    <div>
                        <label class="block text-gray-700 dark:text-gray-300"><?php echo TranslationHelper::translateIfNeeded('Customer Name'); ?></label>
                        <select id="customers_id" name="customers_id" class="w-full mt-1 p-2 border dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 rounded-md focus:ring focus:ring-blue-500">
                            ...
                            <!--[if BLOCK]><![endif]--><?php if(App\Models\customer::where('teams_id', auth()->user()->teams()->first()->id)->count() == null): ?>
                                <option value="" disabled><?php echo TranslationHelper::translateIfNeeded('SNo Customer Found'); ?></option>
                            <?php else: ?>
    ...
    $(document).ready(function() {
        $('#triggerButton').click(function() {
            ...
            if (caseValue.trim() === '') {
                ...
                $.ajax({
                    url: '<?php echo e(route('create-project')); ?>',
                    type: 'POST',
                    data: {
...
<?php /**PATH C:\laragon\www\drone-app-arcom\resources\views/component/button-project.blade.php ENDPATH**/ ?>